...
type CaseStore interface {
	...
	// Delete case
	Delete(req *opts.DeleteOptions) error
...
type CaseCommunicationStore interface {
	...
	Unlink(*opts.DeleteOptions) (int64, error)
...
type StatusStore interface {
	...
	Create(rpc options.CreateOptions, add *_go.Status) (*_go.Status, error)
	// List status lookup
	List(rpc *model.SearchOptions) (*_go.StatusList, error)
	// Delete status lookup
	Delete(rpc *opts.DeleteOptions) error
	// Update status lookup
	Update(rpc options.UpdateOptions, lookup *_go.Status) (*_go.Status, error)
}
...
type StatusConditionStore interface {
	...
	Delete(ctx *opts.DeleteOptions, statusId int64) error
...
type CloseReasonGroupStore interface {
	...
	Create(rpc options.CreateOptions, add *_go.CloseReasonGroup) (*_go.CloseReasonGroup, error)
	...
	Update(rpc options.UpdateOptions, lookup *_go.CloseReasonGroup) (*_go.CloseReasonGroup, error)
}

type CloseReasonStore interface {
	// Create a new reason
	Create(ctx options.CreateOptions, add *_go.CloseReason) (*_go.CloseReason, error)
	// List reasons
	List(ctx *model.SearchOptions, closeReasonId int64) (*_go.CloseReasonList, error)
	// Delete reason
	Delete(ctx *opts.DeleteOptions, closeReasonId int64) error
	// Update reason
	Update(ctx options.UpdateOptions, lookup *_go.CloseReason) (*_go.CloseReason, error)
}
...
type SLAStore interface {
	...
	Create(rpc options.CreateOptions, add *_go.SLA) (*_go.SLA, error)
	// List SLA lookup
	List(rpc *model.SearchOptions) (*_go.SLAList, error)
	// Delete SLA lookup
	Delete(rpc *opts.DeleteOptions) error
	// Update SLA lookup
	Update(rpc options.UpdateOptions, lookup *_go.SLA) (*_go.SLA, error)
}
...
type SLAConditionStore interface {
	...
	Delete(ctx *opts.DeleteOptions) error
