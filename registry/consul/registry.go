...
func NewConsulRegistry(config *conf.ConsulConfig) (*ConsulRegistry, error) {
	...
	if config.Id == "" {
		return nil, cerr.NewInternalError("consul.registry.new_consul.check_args.service_id", "service id is empty! (set it by '-id' flag)")
	}
	ip, port, err := net.SplitHostPort(config.PublicAddress)
	if err != nil {
		return nil, cerr.NewInternalError("consul.registry.new_consul.parse_address.error", "unable to parse address")
	}
	parsedPort, err := strconv.Atoi(port)
	if err != nil {
		return nil, cerr.NewInternalError("consul.registry.new_consul.parse_ip.error", "unable to parse ip")
	}
	...
	if err != nil {
		return nil, cerr.NewInternalError("consul.registry.new_consul_registry.consulapi_creation.error", err.Error())
	}
...
func (c *ConsulRegistry) Register() error {
	...
	if err != nil {
		return cerr.NewInternalError("consul.registry.consul.register.error", err.Error())
	}
	var checks map[string]*consulapi.AgentCheck
	if checks, err = c.client.Agent().Checks(); err != nil {
		return cerr.NewInternalError("consul.registry.consul.register.get_checks.error", err.Error())
	}
...
func (c *ConsulRegistry) Deregister() error {
	...
	if err != nil {
		return cerr.NewInternalError("consul.registry.consul.deregister.error", err.Error())
	}
	c.stop <- true
